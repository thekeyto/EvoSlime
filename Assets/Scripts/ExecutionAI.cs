...
//		BTConfiguration.ENABLE_DATABASE_LOG = true;
		...
		{
			FindEscapeDestination findDestination = new FindEscapeDestination(Constants.GO_Player, Constants.DESTINATION, sightForPlayer);
		...
		BTSequence fight = new BTSequence(shouldFight);
		{
			...
			{
				FindToTargetDestination findToTargetDestination = new FindToTargetDestination(Constants.GO_Grass, Constants.DESTINATION, fightDistance * 0.9f);
			...
			CheckInSight checkGoblinInFightDistance = new CheckInSight(fightDistance, Constants.GO_Grass);	// precondition
